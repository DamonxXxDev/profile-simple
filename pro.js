...
client.pointsMonitor = (dateformat, message) => {
  ...
  if (score.level < curLevel) {
        message.channel.send(`Keep Up <@!${message.author.id}> You have upgraded ${curLevel} `);
...
message.channel.stopTyping(1)
 ...
 if(message.content.startsWith(prefix + 'note')) {
    ...
    const embed = new Discord.RichEmbed()
      .setTitle('خطأ!')
      .setColor('RED')
      .setDescription('**:alarm_clock: | you already rep ' + moment().endOf('day').fromNow().replace('in ', 'next ') + '**')
...
var prefix =`!`;
  let command = message.content.split(" ")[0];
      if (command === prefix + "set") {
        if(!args[0]) return message.reply('choose number BackGround')
        ...
        message.reply('DONe you Buy background');
        } else {
            message.reply('You dont the BackGround ')
        }
    }

    if(message.content.startsWith(prefix + 'bg')) {
...
.setDescription('**for  Buy BackGround Type -buy and choose number BackGround ** ')
...
.addField('Profile Old Tree','Price: $4000 Number: 5')
 .setImage("");
            ...
            if(wallpapers[args[0]].price > row.points) {
                message.reply('You dont have Mony For Buy')
            } else {
                if(dataPro[message.author.id].walls == wallpapers[args[0]]) return message.reply('You already Have The BackGround ');
                else {
                    ...
                    message.reply('DONe you Buy background Type -Set to choose '+args[0]);
                }
